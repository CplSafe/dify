...
| 返点配置 | `api/controllers/console/creator/rebate.py` | 超管读写 `RebateConfig` |
| 返点记录 | `api/controllers/console/creator/rebate.py` | 用户查自己的返点账单 + 超管作废待结算记录 |
| 返点结算/解冻定时任务 | `api/schedule/rebate_settlement_task.py`、`api/schedule/rebate_unfreeze_task.py` | Celery beat 按 `RebateConfig.settlement_hour` 执行 |
| 数据表 | `AccountInvitation`、`RebateRecord`、`RebateConfig`、`UserBalance.rebate_pending` | 未删除，迁移已上线 |
...
2. **恢复"邀请管理" tab**
   `web/app/components/creator/settings/creator-settings-modal.tsx` 把
   `invitation` 菜单项的 `hidden: true` 删掉即可。

3. **（可选）恢复左下角余额徽章**
   见 `user-menu.tsx` 顶部注释，把 `loadBalance` 逻辑加回来。和本主题无关，单独评估。
...
- **不要**因为前端隐藏了 UI，就去删后端路由或服务。后端被很多测试和定时任务依赖。
- **不要**把 `InvitationTab` 组件和 `invite_code` 相关的 payload 字段清理掉，属于"未使用但保留"的状态，和 knip 报告里的 `unused exports` 一类处理。
- 分享链接 `/signup?invite_code=XXX` 进入的用户仍会正常绑定邀请关系，这是产品决定的保留行为。
