...
export function proxy(request: NextRequest) {
  ...
  if (hiddenTopLevelRoutes.some(route => pathname === route || pathname.startsWith(`${route}/`))) {
  ...
  const hiddenAppDetailMatch = pathname.match(/^\/app\/([^/]+)\/(workflow|configuration)(?:\/.*)?$/)
  if (hiddenAppDetailMatch) {
    const url = request.nextUrl.clone()
    url.pathname = `/app/${hiddenAppDetailMatch[1]}/develop`
    url.search = ''
    return NextResponse.redirect(url)
  }
