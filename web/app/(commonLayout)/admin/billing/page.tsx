...
export default function AdminBillingPage() {
  ...
  const [activeTab, setActiveTab] = useState<'balances' | 'records'>(
    'balances',
  )
  ...
  const handleTopup = async () => {
    ...
    try {
      ...
      if (resp.ok) {
        ...
        // Refresh balances
        const data = await fetch('/console/api/creator/admin/balances').then(
          r => r.json(),
        )
  ...
  return (
    <div className="flex h-full flex-col overflow-y-auto">
      <div className="border-b border-divider-subtle px-8 py-6">
        <h1 className="text-2xl font-bold text-text-primary">账户积分管理</h1>
      ...
      <div className="flex-1 space-y-8 p-8">
        ...
        <div className="rounded-2xl border border-divider-subtle bg-background-default p-6">
          <h2 className="mb-4 flex items-center gap-2 font-semibold text-text-primary">
            <RiAddLine className="text-primary-600 h-5 w-5" />
            为用户充值
          </h2>
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
            <div>
              ...
              <input
                ...
                onChange={e =>
                  setTopupForm(p => ({ ...p, account_id: e.target.value }))}
            ...
            <div>
              ...
              <input
                ...
                onChange={e =>
                  setTopupForm(p => ({ ...p, description: e.target.value }))}
                placeholder="充值备注（可选）"
                className="w-full rounded-lg border border-components-input-border-active bg-components-input-bg-normal px-3 py-2 text-sm text-text-primary outline-none placeholder:text-text-quaternary focus:border-primary-500"
              />
            </div>
          ...
          <div className="mt-4 flex items-center gap-3">
            <button
              onClick={handleTopup}
              disabled={
                topupLoading || !topupForm.account_id || !topupForm.amount
              }
            ...
            {topupMessage && (
              <span
                className={`text-sm ${topupMessage.includes('成功') ? 'text-green-600' : 'text-red-600'}`}
              >
                {topupMessage}
              </span>
            )}
        ...
        {loading
          ? (
              <div className="py-12 text-center text-text-quaternary">
                加载中...
              </div>
            )
          : activeTab === 'balances'
                  ...
                  <table className="w-full text-sm">
                    ...
                    <tbody className="divide-y divide-divider-subtle">
                      {balances.map(b => (
                        <tr
                          key={b.account_id}
                          className="hover:bg-background-default-dimm"
                        >
                          ...
                          <td className="px-4 py-3 font-mono text-xs text-text-quaternary">
                            {b.account_id}
                          </td>
                          <td className="px-4 py-3 text-right">
                            <span
                              className={`font-semibold ${Number.parseFloat(b.balance) < 0 ? 'text-red-600' : 'text-text-primary'}`}
                            >
                          ...
                          <td className="px-4 py-3 text-center">
                            <span
                              className={`rounded-full px-2 py-0.5 text-xs font-medium ${b.is_sufficient ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}
                            >
                      ...
                      {balances.length === 0 && (
                        <tr>
                          <td
                            colSpan={4}
                            className="px-4 py-8 text-center text-text-quaternary"
                          >
                            暂无用户积分记录
                          </td>
                        </tr>
                    ...
                    <tbody className="divide-y divide-divider-subtle">
                      {records.map(r => (
                        <tr key={r.id} className="hover:bg-background-default-dimm">
                          <td className="px-4 py-3">
                            <span
                              className={`flex w-fit items-center gap-1 rounded-full px-2 py-0.5 text-xs font-medium ${r.record_type === 'topup' ? 'bg-green-100 text-green-700' : 'bg-orange-100 text-orange-700'}`}
                            >
                              {r.record_type === 'topup'
                                ? (
