...
export default function CreatorPage() {
  ...
  const [initialDraft, setInitialDraft] = useState<CreatorChatDraft | null>(null)
  const [homeAppParams, setHomeAppParams] = useState<any>(null)
  const [workspaceSessionKey, setWorkspaceSessionKey] = useState(() => crypto.randomUUID())
  const [workspaceIsFreshConversation, setWorkspaceIsFreshConversation] = useState(() => !appId)
  ...
  const handleOpenWorkspace = useCallback((draft?: Omit<CreatorChatDraft, 'id'>) => {
  ...
  return (
    <div className="relative flex h-full min-h-0 flex-col overflow-hidden bg-[#FBFBFF]">
      ...
      <div className={cn('relative z-10 min-h-0 flex-1', showWorkspace && 'px-2 pb-2')}>
        {!showWorkspace && (
        ...
        {showWorkspace && (loadingApps || installingAppId || !installedAppId)
          ? (
        ...
        {showWorkspace && installedAppId && !loadingApps && !installingAppId && (
