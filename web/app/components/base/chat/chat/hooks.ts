...
export const useChat = (
  ...
  const buildChatErrorMessage = useCallback(
  ...
  const updateChatTreeNode: UpdateChatTreeNode = useCallback(
    (
      id: string,
      fieldsOrUpdate:
        | Partial<ChatItemInTree>
        | ((node: ChatItemInTree) => void),
    ) => {
    ...
    async (
      ...
      const otherOptions: IOtherOptions = {
          ...
          updateChatTreeNode(messageId, (responseItem) => {
            const isAgentMode
              = responseItem.agent_thoughts
                && responseItem.agent_thoughts.length > 0
            if (!isAgentMode) {
        ...
        onFile(file) {
          ...
          const baseFile
            = 'transferMethod' in file ? (file as Partial<FileEntity>) : null
          ...
          updateChatTreeNode(messageId, (responseItem) => {
            ...
            if (lastThought) {
              ...
              const currentFiles
                = (responseItem.message_files as FileEntity[] | undefined) ?? []
        ...
        onMessageEnd: (messageEnd) => {
          updateChatTreeNode(messageId, (responseItem) => {
            ...
            const processedFilesFromResponse = getProcessedFilesFromResponse(
              messageEnd.files || [],
            )
            responseItem.allFiles = uniqBy(
              [
                ...(responseItem.allFiles || []),
                ...(processedFilesFromResponse || []),
              ],
              'id',
            )
          })
        ...
        onWorkflowStarted: ({ workflow_run_id, task_id }) => {
          ...
          updateChatTreeNode(messageId, (responseItem) => {
            if (
              ...
              && responseItem.workflowProcess.tracing.length > 0
            ) {
          ...
          updateChatTreeNode(messageId, (responseItem) => {
            if (!responseItem.humanInputFormDataList) {
              ...
              const currentFormIndex
                = responseItem.humanInputFormDataList.findIndex(
                  item => item.node_id === humanInputRequiredData.node_id,
                )
              if (currentFormIndex > -1) {
                responseItem.humanInputFormDataList[currentFormIndex]
                  = humanInputRequiredData
              }
              else {
                responseItem.humanInputFormDataList.push(
                  humanInputRequiredData,
                )
              }
            }
            if (responseItem.workflowProcess?.tracing) {
              const currentTracingIndex
        ...
        onHumanInputFormFilled: ({ data: humanInputFilledFormData }) => {
          updateChatTreeNode(messageId, (responseItem) => {
            ...
            if (!responseItem.humanInputFilledFormDataList) {
              responseItem.humanInputFilledFormDataList = [
                humanInputFilledFormData,
              ]
            }
            else {
              responseItem.humanInputFilledFormDataList.push(
                humanInputFilledFormData,
              )
            }
          })
          ...
          updateChatTreeNode(messageId, (responseItem) => {
            if (responseItem.humanInputFormDataList?.length) {
              ...
              responseItem.humanInputFormDataList[
                currentFormIndex
              ].expiration_time = humanInputFormTimeoutData.expiration_time
            }
          })
        },
  ...
  const handleSend = useCallback(
    async (
      ...
      const parentMessage = threadMessages.find(
        item => item.id === data.parent_message_id,
      )
      ...
      const conversationId
        = data.conversation_id || conversationIdRef.current || undefined
      const bodyParams = {
      ...
      const otherOptions: IOtherOptions = {
        ...
        async onCompleted(hasError?: boolean) {
          ...
          if (
            ...
            const responseLog = Array.isArray(newResponseItem.message)
            ...
            const providerLatency = Number(
              newResponseItem.provider_response_latency,
            )
            const hasValidProviderLatency
              = Number.isFinite(providerLatency) && providerLatency >= 0
            const isUseAgentThought
              = newResponseItem.agent_thoughts?.length > 0
                && newResponseItem.agent_thoughts[
                  newResponseItem.agent_thoughts?.length - 1
            ...
            updateChatTreeNode(responseItem.id, {
              ...
              more: {
                ...
                latency: hasValidProviderLatency
                  ? providerLatency.toFixed(2)
                  : undefined,
                tokens_per_second:
                  hasValidProviderLatency && providerLatency > 0
                    ? (answerTokens / providerLatency).toFixed(2)
                    : undefined,
              },
              // for agent log
              conversationId: conversationIdRef.current,
            ...
            await onMessageCompleted?.({
              ...
              messageId: newResponseItem.id,
              workflowRunId:
                newResponseItem.workflow_run_id || responseItem.workflow_run_id,
              conversationId: conversationIdRef.current,
            })
          }
          if (
  ...
  const handleSwitchSibling = useCallback(
    (siblingMessageId: string, callbacks: SendCallback) => {
      ...
      const targetMessage = findMessageInTree(
        chatTreeRef.current,
        siblingMessageId,
      )
      if (
