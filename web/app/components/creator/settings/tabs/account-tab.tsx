...
// Creator-specific account tab — same as AccountPage but without the
// app-list section (which calls /apps and is irrelevant for creators).
...
export default function AccountTab() {
  ...
  const valid = () => {
    if (!password.trim()) { toast.error(t('error.passwordEmpty', { ns: 'login' })); return false }
    if (!validPassword.test(password)) { toast.error(t('error.passwordInvalid', { ns: 'login' })); return false }
    if (password !== confirmPassword) { toast.error(t('account.notEqual', { ns: 'common' })); return false }
    return true
  }
  ...
  const handleSavePassword = async () => {
    ...
    try {
      ...
      await updateUserProfile({
        ...
        body: { password: currentPassword, new_password: password, repeat_new_password: confirmPassword },
      })
      toast.success(t('actionMsg.modifiedSuccessfully', { ns: 'common' }))
      mutateUserProfile()
    ...
    <>
      ...
      {editPasswordModalVisible && (
        <Dialog open={editPasswordModalVisible} onOpenChange={open => !open && (setEditPasswordModalVisible(false), resetPasswordForm())}>
          <DialogContent className="w-[420px]! p-6!">
            <div className="mb-6 text-text-primary title-2xl-semi-bold">
              {userProfile.is_password_set ? t('account.resetPassword', { ns: 'common' }) : t('account.setPassword', { ns: 'common' })}
            </div>
              ...
              <>
                ...
                <div className="relative mt-2">
                  ...
                  <div className="absolute inset-y-0 right-0 flex items-center">
                    <Button type="button" variant="ghost" onClick={() => setShowCurrentPassword(!showCurrentPassword)}>
                      {showCurrentPassword ? '👀' : '😝'}
                    </Button>
                  </div>
                </div>
            ...
            <div className="relative mt-2">
              ...
              <div className="absolute inset-y-0 right-0 flex items-center">
                <Button type="button" variant="ghost" onClick={() => setShowPassword(!showPassword)}>{showPassword ? '👀' : '😝'}</Button>
              </div>
            </div>
            ...
            <div className="relative mt-2">
              ...
              <div className="absolute inset-y-0 right-0 flex items-center">
                <Button type="button" variant="ghost" onClick={() => setShowConfirmPassword(!showConfirmPassword)}>{showConfirmPassword ? '👀' : '😝'}</Button>
              </div>
            </div>
            <div className="mt-10 flex justify-end">
              <Button className="mr-2" onClick={() => { setEditPasswordModalVisible(false); resetPasswordForm() }}>{t('operation.cancel', { ns: 'common' })}</Button>
      ...
      {showUpdateEmail && (
        <EmailChangeModal show={showUpdateEmail} onClose={() => setShowUpdateEmail(false)} email={userProfile.email} />
      )}
    </>
  )
}
