...
const InviteModal = ({ isEmailSetup, onCancel, onSend }: IInviteModalProps) => {
  ...
  const refreshLicenseLimit = useProviderContextSelector(
    s => s.refreshLicenseLimit,
  )
  const [emails, setEmails] = useState<string[]>([])
  const [isLimited, setIsLimited] = useState(false)
  const [isLimitExceeded, setIsLimitExceeded] = useState(false)
  const [usedSize, setUsedSize] = useState(
    licenseLimit.workspace_members.size ?? 0,
  )
  ...
  }, [licenseLimit, emails.length])

  const locale = useLocale()
  const [role, setRole] = useState<RoleKey>('normal')

  const [isSubmitting, { setTrue: setIsSubmitting, setFalse: setIsSubmitted }]
    = useBoolean(false)
  ...
  const handleSend = useCallback(async () => {
    ...
    try {
      if (
        emails.map((email: string) => emailRegex.test(email)).every(Boolean)
      ) {
      ...
      >
        ...
        <div>
          ...
          <div className="mb-8 flex h-36 flex-col items-stretch">
            <ReactMultiEmail
              className={cn(
                'h-full w-full border-components-input-border-active bg-components-input-bg-normal! px-3 pt-2 outline-hidden',
              ...
              getLabel={(email, index, removeEmail) => (
                <div
                  data-tag
                  key={index}
                  className={cn('bg-components-button-secondary-bg!')}
                >
          ...
          <div
            className={cn(
              'flex items-center justify-end text-text-tertiary system-xs-regular',
